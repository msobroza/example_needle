#!/usr/bin/env bash
# Domino App entry point: OpenVINO Model Server serving the reranker
# (bge-reranker-base, int8) on 0.0.0.0:8888 as POST /v3/rerank.
...
CONFIG="$MODELS_DIR/config_reranker.json"

if [ ! -f "$CONFIG" ]; then
  echo "app_ovms_reranker.sh: OVMS config not found: $CONFIG" >&2
...
if ! command -v ovms >/dev/null 2>&1; then
  echo "app_ovms_reranker.sh: 'ovms' binary not on PATH; build the Domino environment from domino/environment/Dockerfile.ovms" >&2
  exit 1
fi

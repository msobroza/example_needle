...
`needle_core.domain` is the **dependency-light domain layer**. It models
...
```mermaid
flowchart TB
    subgraph domain["needle_core.domain"]
...
`needle_core.domain.document.document`.
...
`needle_core.domain.document.discovery` walks a directory tree and turns
...
`needle_core.domain.document.checksum` provides streamed SHA-256 helpers
...
`needle_core.domain.metadata.schema` declaratively validates a metadata
...
`needle_core.domain.identifiers` provides typed, self-describing
...
```python
from needle_core.domain.identifiers import DocumentId, VersionId, QueryId
...
`needle_core.domain.ranking` models scores and ranked results. A `Score`
...
```python
from needle_core.domain.ranking import Score, RankedPage, Ranking
...
`needle_core.domain.pagination` provides offset/limit pagination. A
...
```python
from needle_core.domain.pagination import PageRequest, Page
...
`needle_core.domain.geometry` defines `BoundingBox`, an axis-aligned
...
`needle_core.domain.events` provides lightweight records describing things
...
```python
from needle_core.domain.events import (
...
`needle_core.domain.interaction.response.RetrievalResponse` bundles the
...
`needle_core.domain.interaction.conversation` models multi-turn dialogue.
A `Turn` pairs a user `Query` with an optional response; a `Conversation` is an
ordered sequence of turns:

```python
from needle_core.domain.interaction.conversation import Conversation
...
`needle_core.domain.ports` declares the abstractions the retrieval layer
...
from needle_core.domain.ports.page_renderer import PageRenderer
